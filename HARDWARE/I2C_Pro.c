...
ErrorStatus I2C_Check_Ack(void)
{
	...
	SCL_H;
	Delay_us(1);		//拉高总线5us
	if(GPIO_ReadOutputDataBit(GPIOB, GPIO_Pin_7)==RESET)		//PB7是否为低电平，应答
	{
...
u8 I2C_Tx(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *data)
{
	int i,flag;
	I2C_Start();								//起始信号
	I2C_SendByte(addr | 0);			//发送器件地址，写->0
	flag=I2C_Check_Ack();						//检测应答
	I2C_SendByte(reg);					//写入首地址
	flag=I2C_Check_Ack();						//检测应答
	for(i=0;i<len;i++)
		I2C_SendByte(data[i]);   	//写入数据		
	flag=I2C_Check_Ack();						//检测应答
	I2C_Stop();									//停止信号
	return flag;
}
...
u8 I2C_Rx(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *buf)
{
	u8 flag;
	I2C_Start();					//起始信号
	I2C_SendByte(addr | 0);	//器件地址，写->0
	flag=I2C_Check_Ack();			//应答
	I2C_SendByte(reg);		//写入首地址
	flag=I2C_Check_Ack();			//检测应答
	I2C_Start();					//起始信号
	I2C_SendByte(addr | 1);	//器件地址，读->1
	flag=I2C_Check_Ack();			//检测应答
	while(len)
	{
		*buf=I2C_ReadByte();			//读出数据
		I2C_Master_Ack(len==1?RESET:SET);	//主动非应答	
		len--;
		buf++;
	}
	I2C_Stop();						//停止信号
